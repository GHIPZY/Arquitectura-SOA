...
import { Bell, Trophy, Users, Calendar, CheckCheck } from 'lucide-react'
import {
  getNotificaciones,
  marcarLeida,
  marcarTodasLeidas,
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <Bell size={18} />
        {noLeidas > 0 && (
          <span className="absolute -top-0.5 -right-0.5 min-w-4.5 h-4.5 px-1 rounded-full bg-accent text-white text-[10px] font-bold flex items-center justify-center">
                ...
                {notificaciones.map(n => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 flex gap-3 transition-colors cursor-pointer ${
                      n.leida ? 'opacity-60 hover:bg-base/50' : 'bg-primary/[0.03] hover:bg-base'
                    }`}
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-subtle mt-1">{tiempoRelativo(n.created_at)}</p>
                    </div>
                    {!n.leida && <span className="w-2 h-2 rounded-full bg-accent shrink-0 mt-1.5 ml-auto" />}
                  </button>
                ))}
              </div>
            )}
